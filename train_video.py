...
from utils.lossL1 import CrossEntropyWithL1
...
def train(train_loader, model, optimizer, epoch, save_path, writer, freq):
    ...
    try:
        for i, data_blob in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}/{opt.epoch}", leave=False), start=1):
            # if i >= 10:
            #     break
        ...
        loss_all /= epoch_step
        end_time = time.time()
        time_spend = end_time - strat_time

        print('[Train Info]: Epoch [{:03d}/{:03d}], Loss_AVG: {:.4f}, Time: {:.2f}s, Lr:{:6f}'.format(epoch, opt.epoch, loss_all, time_spend, optimizer.param_groups[0]['lr']))
        logging.info('[Train Info]: Epoch [{:03d}/{:03d}], Loss_AVG: {:.4f}, Time: {:.2f}s'.format(epoch, opt.epoch, loss_all, time_spend))
        ...
        torch.save(model.state_dict(), save_path + 'Best_Dice.pth'.format(epoch))
    ...
    with ((torch.no_grad())):
        ...
        for i in tqdm(range(test_loader.size)):
            ...
            if case_idx == 0:
                # print('case_idx == 0')
                model.mem_bank.clear_memory()
                res = model(images,[case_idx],mode='val', use_mem=False, add_mem=True, clear=True)[-1]
            elif (case_idx % mem_batch_freq == 0) and case_idx!=0:
                # print('(case_idx % mem_batch_freq == 0) and case_idx!=0')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--decay_rate', type=float, default=0.6, help='decay rate of learning rate')
    ...
    parser.add_argument('--mem_freq', type=int, default=8, help='mem every n frames')
    parser.add_argument('--test_mem_length', type=int, default=50, help='max num of memory frames')
    
    parser.add_argument('--save_path', type=str,default='./snapshot/PolypMoE/',
    ...
    # skip_list = [15, 15, 15, 15, 15]
    skip_i = 0

    torch.backends.cudnn.enabled = False

    print("Data 6/03 Start train...")
    for epoch in range(1, opt.epoch):
        ...
        if epoch % 5 == 0:
            train_loader = get_loader(dataset_root=opt.dataset_root,
