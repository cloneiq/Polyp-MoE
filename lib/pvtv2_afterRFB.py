...
class RFB_modified(nn.Module):
    """
        四个分支包含不同的卷积和膨胀操作，以获取不同尺度的特征并增强模型的多尺度感知能力
    """
...
class NeighborConnectionDecoder(nn.Module):
    """
        用于解码器，对特征图上采样并进行融合，将不同层次的特征图进行融合
    """
    # used after MSF
...
class GRA(nn.Module):
    """
        GRA 是一个注意力机制，它将输入特征和一组分数(score)结合起来，通过卷积和ReLu激活函数调整特征，然后通过另一个卷积层来计算注意力分数。
    """
...
class FeatureExtraction(nn.Module):
    def __init__(self, channel=32, pvtv2_pretrained_path=None, size_img=224):
        ...
        if pvtv2_pretrained_path is not None:
            assert os.path.exists(pvtv2_pretrained_path)
            self.load_model(pvtv2_pretrained_path) 
            print('load pvt from:', pvtv2_pretrained_path)
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        Ss = [S_5,S_4,S_3]
        return S_g_pred, S_5_pred, S_4_pred, S_3_pred, Ss


class Network(nn.Module):
    # res2net based encoder decoder
    def __init__(self, channel=32, pvtv2_pretrained_path=None, imgsize=224):
        super(Network, self).__init__()
        self.channel = channel
        self.imgsize = imgsize
        self.pvtv2_pretrained_path = pvtv2_pretrained_path
        self.feat_net = FeatureExtraction(self.channel, self.pvtv2_pretrained_path, self.imgsize)
